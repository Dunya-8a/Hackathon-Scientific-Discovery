...
def _augment_references(base: str, literature: list[dict]) -> str:
    ...
    if not literature:
        return base
    # Count existing numbered refs in `base` to continue numbering.
    n_existing = len(re.findall(r"^\s*\d+\.\s", base, flags=re.MULTILINE))
    lines = ["", "## Retrieved literature", ""]
    for offset, h in enumerate(literature, start=1):
        lines.append(f"{n_existing + offset}. {h['title']}. {h['url']}")
    return base.rstrip() + "\n\n" + "\n".join(lines).lstrip()
...
def _format_log_for_results(log: list[dict]) -> str:
    """Render the log as a compact JSONL-ish text for the Results prompt."""
    out = []
    for e in log:
        out.append(
            f'{{"id": {e["id"]}, "kind": "{e["kind"]}", '
            f'"plan": {e.get("plan", "")!r}, '
            f'"metric": {e["metric"]}, '
            f'"ok": {e["ok"]}, '
            f'"kept": {e.get("kept", "—")}}}'
